    ...
    if (window.location.href.includes('/signup')) {
        ...
        inputEmail.onkeyup = async function() {
            ...
            if (!emailRegex.test(emailValue)) {
                ...
                const url = `http://127.0.0.1:8200/api/users/check-email?email=${encodeURIComponent(emailValue)}`;
                try {
    ...
    if (window.location.href.includes('/signup') || window.location.href.includes('/profile-edit1')) {
        ...
        inputNickname.onkeyup = async function () {
            ...
            if (inputNickname.value.length === 0) {
                ...
                // 닉네임 중복 검증
                const url = `http://127.0.0.1:8200/api/users/check-nickname?nickname=${encodeURIComponent(inputNickname.value)}`;
                try {
                    const response = await fetch(url, {
                        method: 'GET',  
                        cache: 'no-cache', 
